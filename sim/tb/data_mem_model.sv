...
import "DPI-C" function void sparse_mem_write_tag( input int unsigned addr, input byte unsigned tag);
import "DPI-C" function int sparse_mem_dump( input string filename);
...
module data_mem_model import kudu_dv_pkg::*; # (
  ...
  parameter bit UseSparseMem   = 1'b0
) (
  input  logic              clk,
  input  logic              rst_n,
  ...
  function automatic logic [64:0] read_mem_dpi (logic [31:0] address);
    logic [31:0] addr64;
    logic        tag;
    logic [7:0][7:0] bytes;
    ...
    tag = sparse_mem_read_tag(addr64);
    for (int i = 0; i < 8; i++) bytes[i] = sparse_mem_read_data(addr64+i);

    return {tag, bytes};
  endfunction
 
  function automatic logic write_mem_dpi (logic [31:0] address, logic [64:0] data, logic is_cap, logic [3:0] be);
    logic [31:0] addr64, addr32;
    logic [7:0]  tag;
    logic [7:0][7:0] bytes;
    ...
    if (is_cap) begin
      ...
      for (int i = 0; i < 4; i++) begin
        if (be[i]) sparse_mem_write_data(addr32+i, bytes[i]);
      end
    end
  ...
  if (UseSparseMem) begin : g_data_rw_dpi
    // QQQ let's worry about lr/sc later
    assign mem_err     = 1'b0;
